...
def _predict_and_log(req: PredictionRequest, role: str, reason: str,
                     endpoint: str) -> tuple[str, float, str]:
    ...
    append_log({
        ...
        "model_version": registry.version(role),
        "client_id": req.client_id,
        "listing_id": f.listing_id,
        "input_features": f.model_dump(),
        "derived_features": derived,
        "predicted_annual_revenue": prediction,
        "latency_ms": round(latency_ms, 3),
        "ground_truth_annual_revenue": None,  # filled offline in A/B evaluation
        "schema_version": SCHEMA_VERSION,
    })
    return request_id, prediction, registry.version(role)
...
def health() -> dict:
    ...
    return {
        ...
        "target": registry.meta.get("target"),
    }
...
def predict_revenue(req: PredictionRequest) -> PredictionResponse:
    ...
    request_id, prediction, _ = _predict_and_log(req, role, reason, "/predict_revenue")
    return PredictionResponse(
        request_id=request_id,
        listing_id=req.features.listing_id,
        predicted_annual_revenue=round(prediction, 2),
    )
...
def predict_revenue_forced(model: str, req: PredictionRequest) -> DebugPredictionResponse:
    ...
    request_id, prediction, version = _predict_and_log(
